...
def check_ranking_and_group(rankings_ids, group_ids):
    ...
    print('this is what is checked', rankings_ids)
...
def check_increase_values(group_ids):
    if set(group_ids) != set(range(0, np.max(group_ids) + 1)):
        return
    # raise Exception("group_ids does not contain integers [0," + str(np.max(group_ids)) + "]")


def check_all_distinct(arrays):
    for i in arrays:
        if len(i) != len(np.unique(i)):
            print('ensure you are ranking all')
            #raise Exception("Array does not contain all unique id values")
